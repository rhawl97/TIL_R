...
library(NbClust)
nc = NbClust(df.ab, min.nc=2, max.nc=15, method="kmeans") #2개~15개 군집 #2개가 가장 많이 voting
table(nc$Best.n[1,])  
par(mfrow=c(1,1))
...
print(fit.df2)
printcp(fit.df2) #가지치기를 어디서 멈출 것인지 확인 #xerror+xstd가 최소일 때
plotcp(fit.df2)
...
par(mfrow = c(1,1))
plot(df$cluster, df$future_recommend, ylab="fu_rec",xlab = "cl" )
plot(df$cluster, df$future_lookfor, ylab="fu_lf",xlab = "cl" )
plot(df$cluster, df$trust, ylab="trust",xlab = "cl" )
plot(df$cluster, df$lookfor, ylab="lookfor",xlab = "cl" )
